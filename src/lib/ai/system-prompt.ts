...
import { actionVisible, featureOn } from "@/lib/features";
import type { AlertActionKind } from "@/types";

/**
 * Frozen instructions first (cache-friendly), volatile context last.
 */
const SCOPE = [
  "buildings, units, tenants, contracts, payments, collection, rent roll, expenses and profitability (NOI)",
  featureOn("budgets") && "budgets",
  featureOn("cashflow") && "cash-flow forecasts",
  featureOn("maintenance") && "maintenance work orders and preventive services",
  "assets",
  featureOn("suppliers") && "suppliers",
  featureOn("inspections") && "inspections",
  featureOn("renovations") && "renovations",
  "alerts, the daily briefing and recent activity",
]
  .filter(Boolean)
  .join(", ");
const WRITE_ACTIONS = (["record_payment", "renew_contract", "mark_as_leaving", "create_work_order", "create_reminder", "approve_work_order", "schedule_service", "settle_deposit", "resolve_alert"] as AlertActionKind[]).filter(actionVisible).join(", ");
...
You answer questions about ${SCOPE} using the tools provided. Sections not listed are not part of this edition — say so if asked about them. The tools are the only source of truth — never invent names, amounts or dates. If no tool can answer, say so plainly and suggest what you can answer instead.
...
- Actions: reading is free; anything that changes data (${WRITE_ACTIONS}) only opens a form or asks the owner to confirm — never claim a change was made. Never delete records, edit payments, change rent or contract dates, refund deposits or close work orders yourself.
