...
- Queries (`src/lib/queries/analytics.ts`): `getPortfolioTrends` (occupancy, rent due vs collected, collection rate, outstanding at month end, operating and CapEx, NOI, maintenance cost, estimated vacancy loss from the gaps between contracts), `getExpirationTimeline` (contracts ending per month split into renewing / leaving / undecided), `getExpenseAnalytics` (by category with year-over-year change, by building with per-unit cost, by supplier, month by month against the same month a year earlier, year over year, largest expenses), `getMaintenanceAnalytics` (jobs and cost by category, monthly reported / completed / cost, average and median resolution by priority, repeat issues, supplier performance, highest-cost assets), and `buildReport` / `buildAllReports` for nine exportable tables: rent roll, tenant balances with arrears aging, payment history, expenses, building P&L / NOI, maintenance history, contracts expiring, asset register, supplier performance.
- Pages: `/analytics` (eight trend charts with 6 / 12 / 24-month and building filters, Excel export), `/analytics/expenses` (KPIs, category vs last year, monthly with last-year line, category / building / supplier / largest tables, year over year), `/analytics/maintenance` (KPIs, cost by category, jobs and cost by month, resolution by priority, repeat issues, top assets, supplier performance). The existing `/analytics/performance` building comparison stays.
